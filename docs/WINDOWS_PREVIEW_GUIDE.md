...
- Product-only mode for judging the phone frame without the external harness.
...
6. Click `Product-only view` and confirm the phone surface still feels complete.
7. Tap Orb once. State should become `JARVIS ready`.
8. Tap Orb again. State should become Listening.
9. Click `Speak: scan this`. State should move to Inspection after the endpoint.
10. Click `Keyboard`. Input should remain above the keyboard.
11. The Mesh control, help icon, orb, state line, input, and Send should remain visible in keyboard mode.
12. Click Look at this, Scan this, Read this, and Detect objects.
13. Detect objects should state that the object model is not installed and text/code scanning remain active.
