...
final class JarvisCameraViewController: UIViewController, AVCapturePhotoCaptureDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    private func buildInterface() {
        statusLabel.text = "Inspection starting. Local camera only."
        ...
        hooksLabel.text = "OCR and barcode scanning run after capture when available. \(JarvisObjectDetectionModel.statusLine())"
    ...
    private func analyzeCapturedImage(_ image: UIImage, bytes: Int) {
        ...
        statusLabel.text = "Analyzing text and codes."
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                ...
                if lines.isEmpty && codes.isEmpty && objects.isEmpty {
                    summary += "\nResults ready. No text or barcode found. \(JarvisObjectDetectionModel.statusLine())"
                } else {
                    summary += "\nResults ready."
                    if !lines.isEmpty {
                    ...
                    if !objects.isEmpty {
                        ...
                        summary += "\nObject detection model missing."
                    }
                }
